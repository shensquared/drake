...
void PrintUsageInstructions(const std::string& executable_name) {
  std::cout
      ...
      << " vehicle_model_file [world_model_files] [" << kDurationFlag
      << " duration_in_seconds]" << std::endl
      << std::endl
      << "Where:" << std::endl
...
std::shared_ptr<RigidBodySystem> CreateRigidBodySystem(
    ...
    ModelInstanceIdTable* model_instance_id_table, double penetration_stiffness,
  ...
  ModelInstanceIdTable vehicle_instance_id_table =
      rigid_body_sys->AddModelInstanceFromFile(argv[1], kQuaternion);
    ...
    if (argc > 6) {
      ...
      for (int ii = 2; ii < argc - 6; ++ii) {
        if (std::string(argv[ii]) == "--duration") {
          if (++ii == argc) {
            throw std::runtime_error(
                "ERROR: Command line option \"--duration\" is not followed by "
                "a "
                "value!");
          }
          if (duration != nullptr) *duration = atof(argv[ii]);
        } else {
      ...
      // SetRigidBodySystemParameters(rigid_body_sys,
      // atof(argv[argc-3]),atof(argv[argc-2]),atof(argv[argc-1]));
...
void SetRigidBodySystemParameters(RigidBodySystem* rigid_body_sys,
  ...
  rigid_body_sys->friction_coefficient =
      friction_coefficient;  // essentially infinite friction
}
...
double ParseDuration(int argc, const char* argv[]) {
  for (int ii = 1; ii < argc; ++ii) {
    if (std::string(argv[ii]) == kDurationFlag) {
      if (++ii == argc) {
        PrintUsageInstructions(argv[0]);
        throw std::runtime_error("ERROR: Command line option \"" +
                                 std::string(kDurationFlag) +
...
void AddFlatTerrainToWorld(
    const std::shared_ptr<RigidBodyTree>& rigid_body_tree, double box_size,
    double box_depth) {
...
CreateVehicleSystem(std::shared_ptr<RigidBodySystem> rigid_body_sys,
  ...
  for (int actuator_idx = 0;
    ...
    // std::cout << "map_driving_cmd_to_x_d" << std::endl;
...
CreateMultiVehicleSystem(
  ...
  auto vehicle_sys = cascade(
      std::allocate_shared<Gain<MultiDrivingCommand1,
                                PDControlSystem<RigidBodySystem>::InputVector>>(
          Eigen::aligned_allocator<
...
VectorXd GetInitialState(const RigidBodySystem& rigid_body_sys) {
  ...
  // std::cout <<   tree->getZeroConfiguration() << std::endl;
  // std::cout << "x0" << std::endl;
  // std::cout << x0<< std::endl;
    // for (int i=0; i<tree->number_of_positions()+tree->number_of_velocities(); i++){
    //     std::cout <<   tree->getStateName(i) << std::endl;
    // }
